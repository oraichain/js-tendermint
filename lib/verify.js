...
function getVoteSignBytes(chainId, vote) {
  ...
  const signBytes = VarBuffer.encode(encodedVote);

  // console.log('Canonical vote: ', canonicalVote, 'encoded', encodedVote.toString('hex'), 'sign bytes', signBytes.toString('hex'));
  // process.exit();
  return signBytes;
}
  ...
  try {
    for (var _iterator3 = commit.signatures[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
      ...
      if (!ed25519.verify(signature, signBytes, pubKey)) {
        ...
        console.log('yup', 'sig', signature.toString('hex'), 'data', signBytes.toString('hex'), 'pubkey', pubKey.toString('hex'));
